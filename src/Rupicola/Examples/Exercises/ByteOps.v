...
Definition byte_or (b1 b2: byte) : byte := x00.
Definition byte_not (b: byte) : byte := x00.
...
Definition combine (b1 b2: byte) (m: byte) :=
  ...
  let/n selected_b2 := byte_not x00 in
  let/n z := byte_or x00 x00 in
  z.
...
Then, if you have completed the definition of `combine`, Rupicola will complain that it does not know how to compile `word_or` and `word_not`::

    DEXPR mem
      (map.of_list
         [("selected_b1", word_of_byte v); ("m", word_of_byte m);
         ("b2", word_of_byte b2); ("b1", word_of_byte b1)]) ?e2 (word_of_byte (byte_not m))

   DEXPR mem
     (map.of_list
        [("selected_b2", word_of_byte v0); ("selected_b1", word_of_byte v);
        ("m", word_of_byte m); ("b2", word_of_byte b2); ("b1", word_of_byte b1)])
     ?e (word_of_byte (byte_or v v0))

We need to provide lemma for both of them.
|*)
